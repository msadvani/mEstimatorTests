function [ F1, I ] = computeConstraint1(q0,c, fNoise, g,kappa,form,lambda,N0,L0,pTol)
...
F1= - q0 - a(q0,c) + sum(xi(q0,c).*(convMeas - proxSigma(convMeas,1./(2*cHat(q0,c)),form,lambda)).^2*dConvMeas) + 2*a(q0,c)*sum(xi(q0,c).*proxSigmaPrime(convMeas,1./(2*cHat(q0,c)),form,lambda)*dConvMeas);
